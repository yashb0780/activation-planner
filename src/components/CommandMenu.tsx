...
export function CommandMenu({ commands, onClose }: { commands: Command[]; onClose: () => void }) {
  ...
  const shown = useMemo(() => {
    ...
    return commands.filter((c) => words.every((w) => `${c.label} ${c.hint ?? ""}`.toLowerCase().includes(w))).slice(0, 50);
  }, [commands, query]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="border-t border-line px-4 py-2 text-xs text-faint">↑ ↓ to move · Enter to run · Esc to close</p>
      </div>
    </div>
  );
}
